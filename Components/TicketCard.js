...
const TicketCard = ({ ticketId }) => {
  ...
  useEffect(() => {
    const fetchTicketDetails = async () => {
      try {
        const response = await fetch(`https://connect-ticketing.work.gd/api/tickets/${ticketId}`);
        ...
        const usernameResponse = await fetch(`https://connect-ticketing.work.gd/user/user/${data.userId}`);
  ...
  const formattedArrivalTime = new Date(ticketDetails.arrivalTime);  
  ...
  const formattedArrivalTimeWithAmPm = formattedArrivalTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) +
  (formattedArrivalTime.getHours() >= 12 ? ' PM' : ' AM');
      ...
      >
        ...
        <View style={styles.textContainerRow}>
          ...
          <View style={{alignItems: 'flex-start'}}>
            ...
            <Text style={styles.ticketText3}>{formattedArrivalTimeWithAmPm}</Text>
          </View>
        </View>
        ...
        <View>
          ...
          <View style={styles.ticketDetails}>
            ...
            <Text style={styles.ticketText1}>:</Text>
            <Text style={styles.ticketText2}>{ticketDetails.boardingPoint}</Text>
          </View>

          <View style={styles.ticketDetails}>
            <Text style={styles.ticketText1}>Dropping Point</Text>
            <Text style={styles.ticketText1}>:</Text>
            <Text style={styles.ticketText2}>{ticketDetails.droppingPoint}</Text>
          </View>
